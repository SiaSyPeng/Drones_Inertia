...
class LoggingExample:
    ...
    def _connected(self, link_uri):
        ...
        self.lg_acc = LogConfig(name="acc", period_in_ms=10)
        self.lg_acc.add_variable('acc.x', 'float')
        self.lg_acc.add_variable('acc.y', 'float')
        self.lg_acc.add_variable('acc.z', 'float')

        # open file
        filename = time.strftime("%Y%m%d-%H%M%S")
        self.f = open('acceleration'+filename+'.txt', 'w')

        # Adding the configuration cannot be done until a Crazyflie is
        # connected, since we need to check that the variables we
        # would like to log are in the TOC.
        try:
            self._cf.log.add_config(self.lg_acc)
            # This callback will receive the data
            self.lg_acc.data_received_cb.add_callback(self._stab_log_data)
            # This callback will be called on errors
            self.lg_acc.error_cb.add_callback(self._stab_log_error)
            # Start the logging
            self.lg_acc.start()
